...
failures — consistent across all 106 `data_center_siting` fields. My output is a memo a
...
1 `failed`. An `absent` field keeping its source and a reason ("point does not intersect
any designated Opportunity Zone tract") rather than being dropped is the difference
between "no" and "we didn't look" — different facts in a permit file. Same instinct in
the `503` on `@airports`, which names the missing column, the consequence and the fix.
Refusing beats answering wrong.
...
`/v1/proximity` returns `credits_charged` in the response body and **nothing else does**
— that one field turns client-side accounting from a modelling exercise into a read,
please put it on every endpoint. And parcel-group fields bill `resolve_credits` **once
per location, not per field**: a fetch of exactly `["parcel_id", "parcel_area_m2"]`
billed 300, not 302. That is not obvious and it changes how you plan a sweep —
`site_selection` is 72 fields, 9 of them parcel-group, so 63 + 300 = 363 per location,
not 72 and not 372. Worth saying next to `parcel_field_group` in the plans payload.
Separately, `credits.used` is eventually consistent and went *backwards* mid-run
(0 → 2 → 160 → 115 → 536). It converges; it is just not a live meter.
...
`MIREYE_MODELED_ENERGY_COST`. Because `fields_unknown` is a hard 400 that kills the
whole request, I validate names locally first — and validating against the published
list silently dropped ten good fields until I unioned the preset expansions in. Publish
them or mark them derived, but the two lists should not disagree. (Related: I had 90 and
54 written down for `data_center_siting` and `site_selection`; they are 106 and 72. And
I assumed a preset called `wildfire`; it is `wildfire_underwrite`. `/v1/meta/fields` is
...
`FAA_NASR` — a hospital helipad. The `@airports` curated set does filter FAA NASR to
public-use airports, and it has been 503 for my entire build. So there is currently no
working way to get a public-use airport distance, which is exactly what a FAR Part 77
stack-height review needs, and everyone siting generation does that review. Either apply
the same filter to the fetch field or rename it "nearest aviation facility".

*`confidence` is a bucket string, not a number* — `high`/`medium`/`low`/`unknown`. I
translate through an explicit table (0.9/0.6/0.3), keep the raw bucket alongside it, and
map `unknown` to null rather than a low score, because "not measured" and "measured
badly" are different claims and collapsing them would be a lie in a permit memo. I would
prefer a number; failing that, publish the mapping so everyone's table is the same one.
(The thing you did get right here: confidence tracks the source, not the value —
...
classification, so in an SO2 or PM2.5 county it cannot pick one. Worse, at a
...
coordinate-level cross-check they are genuinely better at, since a FIPS table cannot
tell me which side of a partial-county boundary a parcel sits on and your polygon can.
...
1. **A `near_miss_confirm` asks for a confirmation the API cannot accept.** It sits at
   ...
   and nothing clears it. `/v1/openapi.json` lists exactly two field-request routes;
...
2. **Generated `field_id`s truncate at 60 characters and collide.** My background
   ...
   9.0 µg/m³, 24-hour is 35 — so if two builds publish under one id, one silently
   overwrites the other. And `resume` names one id for the whole request, so I cannot
...
3. **Nothing signals how load-bearing `example_locations` is.** It is `minItems: 1` in
   ...
   and that was the single thing that broke on its first live run. It turns out to be
   the most important field in the payload — it is where near-miss samples get taken and
   what the build is verified against, and picking the right coordinate is what made the
   near-miss reason above useful to me instead of generic. That belongs in the first
   sentence of the endpoint docs, not in the schema.
...
nearest *gauged* stream, a different thing — and label it as a proxy. Cooling-water
...
form collapses that into one call — and gives you one clean thing to price.
...
interface with Mireye as the US implementation precisely so a second country is a new
file rather than a rewrite. The architecture is already waiting for you. The day you go
